...
<?= $this->render('_images_banner_form', ['model' => $model, 'bannersImages' => new common\models\BlocksBannersImages()]); ?>




<?php
if($bannersImages = \common\models\BlocksBannersImages::find()->where(['block_id' => $model->id])->all()) {
    ...
    <div class="banners_carousel">
      <div class="banners_carousel--info">
        ...
        <div class="banners_carousel--info--image">
          <?php
          if($item->files->thumbnail??false) {
            echo '<img src="'.(Yii::$app->params['imageUrl'].$item->files->thumbnail).'">';
          }
          ?>
        </div>
      </div>
      <div >
        <a href="/blocks/<?= $item->id ?>/banners-images-delete" class="menu-link px-3" data-kt-ecommerce-product-filter="delete_row">
